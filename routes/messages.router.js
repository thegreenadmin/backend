...
const { fetchUserMessageHeads, fetchUserMessages, userSendMessage, deleteUserMessageHead, contactStoreMessages } = require("../controllers/message.controller");
const { sendConflictResponse, sendOkResponse, sendCreatedResponse } = require("../utils/response.util");
...
router.get('/inbox', userAuth, async function(req, res) {
    try{
        const messageHeads = await fetchUserMessageHeads(req.query, req.payload.user.id);
...
router.delete('/delete', userAuth, async function(req, res) {
    try{
        const deleteMessage = await deleteUserMessageHead(req.body, req.payload.user.id);

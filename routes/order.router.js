...
router.post("/cancel/create", userAuth, async function (req, res) {
  try {
    const orderCancel = await createCancelOrderRequest(
      req.body,
      req.payload.user.id
    );
...
router.post("/ready/pickup", userAuth, async function (req, res) {
  try {
    const order = await createUserReadyPickupOrder(
      req.body,
      req.payload.user.id
    );
...
router.post("/item/return/create", userAuth, async function (req, res) {
  try {
    const returnItems = await createReturnOrderItemsRequest(
      req.body,
      req.payload.user.id
    );
    sendCreatedResponse(
      res,
      returnItems,
      "Your return request has been placed. We'll get back to you shortly on this."
    );
  } catch (err) {
    sendConflictResponse(res, {}, err);
  }
...
router.post("/return/cancel", userAuth, async function (req, res) {
  try {
    const orderCancel = await cancelReturnOrderRequest(
      req.body,
      req.payload.user.id
    );

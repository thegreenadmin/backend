...
const createCategory = async function(data) {
    ...
    try{
        const {store_id, parent_category_id, category_name, image_url, is_featured_category} = data;
        const __CAREGORY = await Category.create({
            store_id, parent_category_id, category_name, image_url, is_featured_category, status: 'active'
        });
...
const listCategories = async function(data, is_enabled = null) {
    try{
        ...
        const featuredCategory = ((is_featured_category == null) || (is_featured_category == undefined)) ? {} : {is_featured_category};
        ...
        const __CAREGORIES = await Category.findAll({
            ...
            order: [['is_featured_category', 'DESC'], ['category_name', 'ASC']],
            include: [
                {
        ...
        return {
            categories: __CAREGORIES.map((category) => {
                ...
                total_products: category.product_categories.filter(pc => pc.product).length
            }})
        }
    }catch(err) { 
        throw err;
        ...
        const __CATEGORY = await Category.findOne(
            {
                ...
                attributes: [
                    ['id', 'category_id'],
                    'category_name', 'image_url', 'is_featured_category'
                ]
            }
        )
...
const editCategory = async function(data) {
    ...
    try{
        const {store_id, category_id, parent_category_id, category_name, image_url, is_featured_category} = data;
        const __CAREGORY = await Category.update({
            store_id, parent_category_id, category_name, image_url, is_featured_category, status: 'active'
        }, {
            where: {id: category_id, store_id, parent_category_id, status: 'active'}
        });
        return {category_id: __CAREGORY.id}
    }catch(err) {
        throw err;
...
const createProduct = async function (data) {
    ...
    try{
        const {store_id, product, product_categories, product_images, product_contents, product_links} = data;
        ...
        const __PRODUCT_CATEGORIES = await ProductCategory.bulkCreate(
            product_categories.map((category) => {return {...category, product_id: __PRODUCT.id, status: 'active'}}),
            {transaction: __SQL_TRANSACTION}
        )


        const __PRODUCT_IMAGES = await ProductImage.bulkCreate(
            product_images.map((img) => {return {...img, product_id: __PRODUCT.id, status: 'active'}}),
            {transaction: __SQL_TRANSACTION}
        )


        const __PRODUCT_CONTENTS = await ProductContent.bulkCreate(
            product_contents.map((content) => {return {...content,product_id: __PRODUCT.id, status: 'active' }}),
            {transaction: __SQL_TRANSACTION}
        )

        const __PRODUCT_LINKS = await ProductLink.bulkCreate(
            product_links.map((links) => {return {...links, product_id: __PRODUCT.id, status: 'active'}}),
            {transaction: __SQL_TRANSACTION}
        )
        ...
        if(parseInt(product.discount_value) > 0) {
            ...
            const __OFFER = await Offer.create({
                ...
                image_url: product_images.length ? product_images[0].image_url : __STORE.image_url,
...
const listStoreProducts = async function(data, user_id = null, showFavouriteProducts = null, showPreviousProducts = null, offer_id = null) {
    ...
    try{
        ...
        const filterObj = CommonController.getFiltersFromArray(filters, 'product_id');
        const store = store_id ? 
        typeof(store_id) == 'object' ? store_id : {store_id}
        : {};
            ...
            [
                {
                    ...
                    required: (showFavouriteProducts != null) ? showFavouriteProducts : false,
                    where: {user_id, status: 'active'},
                    attributes: [
                        ['id', 'favourite_product_id'],
                        'status'
                    ]
                }
            ] : [];

        const previosOrderedProducts = (showPreviousProducts && user_id) 
        ...
        const previousProductJoin = previosOrderedProducts == null 
            ? {}
            :  previosOrderedProducts.length == 0 ? {id: null} : {id: {[Op.in]: previosOrderedProducts.map((oi) => oi.product_id)}}
        ...
        if(offer_id) {
            const __OFFER = await Offer.findOne({where: {id: offer_id, store_id, is_expired: false, status: 'active'}})
            if(__OFFER) {
                if(!__OFFER.is_offer_for_store) {
                    offerJoinQuery = [{model: OfferProduct, required: true, attributes: [], where: { offer_id, status: 'active'}}]
                }
            }
        }
        ...
        const query = {
            where: {
                ...
                ...store, ...filterObj, ...whereEnabled, ...previousProductJoin, status: 'active'
            },
            include: [
                {
                    model: Store,
                ...
                },
                {
                    ...
                    attributes: [
                        ...
                        'heading', 'paragraph', 'order', 'status'
                    ]
                },
        ...
        const __PRODUCTS = await CommonController.getPaginationResult({
            Model: Product, query, page, page_size, order, as: 'data'
        })
        ...
        const products = __PRODUCTS.data.map((p) => {
            ...
            if(user_id) {
                ...
                const is_favourite_product = (product?.favourite_products?.length) ? true : false;
                const is_previous_product = (product?.previous_products?.length) ? true : false;
            ...
            else if(showPreviousProducts != null || showFavouriteProducts != null) {
                product = Object.assign(
                    {
                        product_id: p.id,
            ...
            product.product_categories = product?.product_categories?.filter((pc) => pc.category)
...
const listStoreUserProducts = async function(data, user_id) {
    ...
    // filters = [{filter_by, filter_value, operation}]
    try{
        const { q, store_id, page, page_size, order_by, order_type, category_id, filters } = data;
        const stores = await (await StoreController.getUserStores(user_id)).stores;
        return listStoreProducts({
            ...data,
            store_id: store_id ? store_id : (stores.length ? {store_id: {[Op.in]: stores.map(store => store.store_id)}} : {store_id: null}), 
        })
    }catch(err) {
        throw err;
    }
...
const getProduct = async function (store_id, product_id, user_id = null, show_reviews = false, show_offer = false) {
    try{
        ...
        if(show_reviews) {
            ...
            const average_rating = __PRODUCT_RATINGS[0] ? __PRODUCT_RATINGS[0].toJSON().average_rating : 0;
        ...
        if(show_offer) {
            ...
            const __OFFERS = await Offer.findAll(
                {
                    ...
                    attributes: [
                        ['id', 'offer_id'],
                        'is_offer_for_store', 'offer_name', 'image_url',
            ...
            if(__STORE_ONLY_OFFER) {
                ...
                const image = S3Controller.getAwsS3SignedFileUrl(__STORE_ONLY_OFFER.image_url);
            ...
            if(offer) {
                product.offer_price = CommonController.getCalculatedPrice(product.selling_price, offer.offer_type, offer.offer_value);
            }else{
                product.offer_price = CommonController.getCalculatedPrice(product.selling_price, product.discount_type, product.discount_value);
            }
            product.offer = offer;
        }
...
const editProduct = async function(data) {
    ...
    try{
        const {store_id, product, product_categories, product_images, product_contents, product_links} = data;
        const {product_id, quantity_type_id, quantity, is_featured_product, product_name,description, product_price, selling_price, discount_type, discount_value,is_product_returnable, return_days_count, length, width, height, weight, is_enabled} = product;
        const __PRODUCT = await Product.findOne({
        ...
        const __UPDATE_PRODUCT = await Product.update(
            {quantity_type_id, quantity, is_featured_product, product_name,description, product_price, selling_price, discount_type, discount_value,is_product_returnable, return_days_count, length, width, height, weight, is_enabled},
            {
                where: {id: product_id, store_id, status: 'active'}, 
                transaction: __SQL_TRANSACTION
            }
        ...
        const __PRODUCT_IMAGES = await ProductImage.findAll({where: {product_id}});

        // Bulk Create/Update/Delete ProductImages
        await CommonController.bulkCreateUpdateDelete(
            ProductImage,
            product_images.map(image => {return {...image, product_id}}),
            'product_image_id',
            {product_id},
            __SQL_TRANSACTION
        )
        ...
        await CommonController.bulkCreateUpdateDelete(
            ProductContent,
            product_contents.map(content => {return {...content, product_id}}),
            'product_content_id',
            {product_id},
            __SQL_TRANSACTION
        )
        ...
        await CommonController.bulkCreateUpdateDelete(
            ProductLink,
            product_links.map(link => {return {...link, product_id}}),
            'product_link_id',
            {product_id},
            __SQL_TRANSACTION
        )
        ...
        if(parseInt(discount_value) > 0) {
            ...
            if(__OFFER && __OFFER.auto_created) {
                ...
                await Offer.update({
                    offer_name: offerName,
                    image_url: __PRODUCT_IMAGES.find((pi) => pi.status == "active" && pi.order == 1) 
                                    ?   __PRODUCT_IMAGES.find((pi) => pi.status == "active" && pi.order == 1).image_url
                                    :   __STORE.image_url
                },{
                    where: {id: __OFFER.id},
                    transaction: __SQL_TRANSACTION
...
const shop_listStoreProducts = async function(data, user_id = null) {
    ...
    // filters = [{filter_by, filter_value, operation}]
    try{
        const { q, store_id, page, page_size, order_by, order_type, category_id, offer_id, filters, is_favourite_products, is_previous_products } = data;
        ...
        const __PRODUCTS = await listStoreProducts(
            ...
            user_id, is_favourite_products, is_previous_products, offer_id
        )
...
const shop_getProductDetails = async function(data, product_id, user_id = null) {
    // data = {store_id, longitude, laititude}
    try{
        // add product view
        const {store_id, longitude, latitude} = data;

        const __PRODUCT = await getProduct(store_id, product_id, user_id, true, true);
        
        if(longitude && latitude && user_id) {

            const address = await USPSController.getAddressUsingLatLong(latitude, longitude);
            if(address.result && address.address) {
                await ProductVisitor.create({
                    user_id, product_id, longitude, latitude, status: 'active',
                    address: address.full_address, city: address.address.city || address.address.town, state: address.address.state, country: address.address.country
                })
            }
        }
        ...
        if(user_id) {
            ...
            if(__PREVIOUS_STORE) {
                await PreviousStore.update({count: __PREVIOUS_STORE.count+1}, {where: {id: __PREVIOUS_STORE.id}})
            }else{
                await PreviousStore.create({count: 1, user_id, store_id, status: 'active'})
            }
            ...
            if(__PREVIOUS_PRODUCT) {
                await PreviousProduct.update({count: __PREVIOUS_PRODUCT.count+1}, {where: {id: __PREVIOUS_PRODUCT.id}})
            }else{
                await PreviousProduct.create({count: 1, user_id, product_id, status: 'active'})
            }
        }
...
const shop_listAppHomeProducts = async function(data, user_id = null) {
    ...
    try{
        const {page, page_size, order_by, order_type, longitude, latitude, mileage} = data;
       ...
       const  getStores  =  await StoreAddress.findAll({
        ...
        let store_id = __ACTIVE_MEMBER_STORES.length ? {
            store_id: {[Op.in]: __ACTIVE_MEMBER_STORES.map(s => {s = s.toJSON(); return s.id;})}
        } : null;
        ...
        if(!store_id) {
            ...
            if(__STORES.length) {
                store_id = {
                    store_id: {[Op.in] : __STORES.map(s => {s = s.toJSON(); return s.id;})}
                }
            }
        ...
        return await shop_listStoreProducts({
            q: '', store_id, page, page_size, order_by, order_type, category_id: null, filters: filters, is_favourite_products: null, is_previous_products: null
        }, user_id)
...
const admin_listCategories = async function(data) {
    ...
    try{
        ...
        const whereStores = stores.length ? {store_id: stores.map(s => s.store_id)} : {};
        const order = order_by && order_type ? [[(order_by == 'category_id' ? 'id' : order_by) , order_type]] : [['id', "DESC"]];
        ...
        const __CATEGORIES = await CommonController.getPaginationResult({
            Model: Category, query, order, page, page_size, as: 'data'
        })
...
const admin_listProducts = async function(data) {
    ...
    try{
        ...
        const order = order_by && order_type ? [[(order_by == 'product_id' ? 'id' : order_by), order_type]] : [['id', 'DESC']]
        ...
        return await CommonController.getPaginationResult({
            Model: Product, query, order, page, page_size, as: 'products'
        })
    }catch(err) {
        throw err;
    }
...
const admin_bulkCreateProducts = async function(data, file) {
    ...
    try{
        const {store_id} = data;
        const __QUANTITY_TYPES = await QuantityType.findAll({where: {status: 'active'}});
        const __CATEGORIES = await Category.findAll({where: {store_id, status: 'active'}})
            ...
            for(let j=0; j<row.cellCount; j++) {
                ...
                if(colName == 'Quantity Type') {
                    ...
                    __QUANTITY_TYPES.forEach((qt) => {
                        if(qt.quantity_type_name == cell.value) {_qt = qt}
                    })
                    if(_qt) {data.product.quantity_type_id = _qt.id}
                }
                ...
                if(colName == 'Featured Product') data.product.is_featured_product = cell.value == '0' ? false : true;
                ...
                if(colName == 'Discount Type') data.product.discount_type = cell.value == 'percentage' ? 'percentage' : 'amount';
                if(colName == 'Discount') data.product.discount_value = cell.value;
                if(colName == 'Is Returnable') data.product.is_product_returnable = cell.value == '0' ? false : true;
                if(colName == 'Return Days') data.product.return_days_count = cell.value;
                ...
                if(colName == 'Is Enabled') data.product.is_enabled = cell.value == '0' ? false : true;

                if(colName == 'Category') {
                    let _c= null;
                    storeCaregories.forEach((c) => {
                        if(c.category_name.toLowerCase() == cell.value.toLowerCase()) {_c = c}
                    })

                    if(_c) {data.product_categories[0].category_id = _c.id}
                    else {
                        const __CATEGORY = await Category.create({ store_id, parent_category_id: null, category_name: cell.value, is_featured_category: false,  status: 'active'})
                        storeCaregories.push(__CATEGORY.toJSON());
                        data.product_categories[0].category_id = __CATEGORY.id
                    }
                };

                if(colName == 'Heading 1') data.product_contents[0].heading = cell.value;
                if(colName == 'Heading 2') data.product_contents[1].heading = cell.value;
                if(colName == 'Heading 3') data.product_contents[2].heading = cell.value;
                if(colName == 'Heading 4') data.product_contents[3].heading = cell.value;
                if(colName == 'Heading 5') data.product_contents[4].heading = cell.value;

                if(colName == 'Paragraph 1') data.product_contents[0].paragraph = cell.value;
                if(colName == 'Paragraph 2') data.product_contents[1].paragraph = cell.value;
                if(colName == 'Paragraph 3') data.product_contents[2].paragraph = cell.value;
                if(colName == 'Paragraph 4') data.product_contents[3].paragraph = cell.value;
                if(colName == 'Paragraph 5') data.product_contents[4].paragraph = cell.value;
                ...
                if(colName == 'Link 1 URL') data.product_links[0].link = cell.value?.hyperlink ? cell.value.hyperlink : cell.value;
                if(colName == 'Link 2 URL') data.product_links[1].link = cell.value?.hyperlink ? cell.value.hyperlink : cell.value;
                if(colName == 'Link 3 URL') data.product_links[2].link = cell.value?.hyperlink ? cell.value.hyperlink : cell.value;
                if(colName == 'Link 4 URL') data.product_links[3].link = cell.value?.hyperlink ? cell.value.hyperlink : cell.value;
                if(colName == 'Link 5 URL') data.product_links[4].link = cell.value?.hyperlink ? cell.value.hyperlink : cell.value;
            }
            if(row.number != 1) {
                datas.push(data)
            };
        ...
        __PRODUCTS.forEach((product, index) => {
            ...
            data.product_images = data.product_images.map(p => {return {...p, product_id: product.id}})
            data.product_contents = data.product_contents.map(p => {return {...p, product_id: product.id}})
            data.product_links = data.product_links.map(p => {return {...p, product_id: product.id}})
            data.product_categories = data.product_categories.map(p => {return {...p, product_id: product.id}})
        })
        ...
        datas.forEach(data => {
            createableCategories = [...createableCategories, ...data.product_categories.filter((pc) => pc.category_id != null)]
            createableContents = [...createableContents, ...data.product_contents.filter((pc) => pc.heading != null && pc.paragraph != null)]
            createableImages = [...createableImages, ...data.product_images.filter((pc) => pc.image_url != null)]
            createableLinks = [...createableLinks, ...data.product_links.filter((pc) => pc.name != null && pc.link != null)]
        })


        if(createableCategories.length) {
            await ProductCategory.bulkCreate(createableCategories, {transaction : __SQL_TRANSACTION})
        }
        if(createableContents.length) {
            await ProductContent.bulkCreate(createableContents, {transaction : __SQL_TRANSACTION})
        }
        if(createableImages.length) {
            await ProductImage.bulkCreate(createableImages, {transaction : __SQL_TRANSACTION})
        }
        if(createableLinks.length) {
            await ProductLink.bulkCreate(createableLinks, {transaction : __SQL_TRANSACTION})
        }
        ...
        for(let i=0; i<__PRODUCTS.length; i++) {
            ...
            const __OFFER = await Offer.create({
                ...
                is_offer_for_store: false,
                offer_name: offerName,
                image_url: data.product_images.length ? data.product_images[0].image_url : __STORE.image_url,
                offer_type: product.discount_type,
                offer_value: product.discount_value,
                is_expired: false,
                status: "active"
...
module.exports = {
    createCategory, editCategory, listCategories,
    getCategoryDetails, createProduct, deleteCategory,
    listStoreProducts, editProduct, getProduct, deleteProduct,
    listQuantityTypes, listStoreUserProducts,



    shop_listStoreCategories, shop_listStoreProducts, shop_getProductDetails,
    shop_listAppHomeProducts, 


    admin_listCategories, admin_listProducts, admin_bulkCreateProducts,

...
const createOfferMessageHeadsAndNotification = async function (store_id, offer_id, message, image_url, __SQL_TRANSACTION) {
    try {
        ...
        if (__FAVOURITE_USERS.length) {
            ...
            for (let i = 0; i < __FAVOURITE_USERS.length; i++) {
                const __IS_PUSH_NOTIFICATION_AVAILABLE = await CommonController.isOpenForNotification(__FAVOURITE_USERS[i].user_id, false, 'offer');
                if(__IS_PUSH_NOTIFICATION_AVAILABLE) {
                    const favUserTokens = await CommonController.getUserTokens(__FAVOURITE_USERS[i].user_id);
                    userTokens = [...userTokens, ...favUserTokens]
                }
            }

            const notificationData = {type: 'offer', offer_id, store_id}
            await SnsController.sendMutliPushNotifications(userTokens, title, message, notificationData);
        }
    } catch (err) {
        throw err
    }
...
const fetchUserMessageHeads = async function (data, user_id) {
    ...
    try {
        ...
        const query = {
            where: { 
                user_id, is_available_for_user: true, status: 'active' ,
        ...
        const __MESSAGE_HEADS = await CommonController.getPaginationResult({
            Model: MessageHead, query, page, page_size, order, as: 'data'
        })
        ...
        const message_heads = __MESSAGE_HEADS.data.map((mh) => {
            ...
            const storeLogo = S3Controller.getAwsS3SignedFileUrl(msgHead.store.logo_url);
            delete msgHead.store.logo_url;
            msgHead.store.logo = storeLogo;

            if(msgHead.user) {
                msgHead.user.image = S3Controller.getAwsS3SignedFileUrl(msgHead.user.image_url);
...
const fetchStoreMessageHeads = async function (data, user_id) {
    ...
    try {
                            ...
                            include: [
                                {
                                    ...
                                    include: [
                                        {
                                            model: Controller,
                                            where: {status: 'active', controller_key: "MANAGE_MESSAGE"}
                                        }
                                    ]
                                }
                            ]
        ...
        const deletedOfferMessageHeads = await MessageHead.findAll({
            where: { 
                offer_id: {[Op.ne]: null}, store_id: {[Op.in]: stores},
                status: 'active'
            },
            include: [
                {
        ...
        const query = {
            where: { 
                is_available_for_store: true, status: 'active' ,
                id: {[Op.notIn]: deletedOfferMessageHeads.map((dMh) => dMh.id)},
                store_id: {[Op.in]: stores}, ...msgReadFilter,
            },
            attributes: {
                include: [['id', 'message_head_id']],
                exclude: ['id']
...
const fetchMessages = async function (data) {
    ...
    try {
        ...
        const __MESSAGES = await CommonController.getPaginationResult({
            Model: Message, page, page_size, order, as: 'data',
            query: {
        ...
        const messages = __MESSAGES.data.map((msg) => {
            ...
            if(msg.sender_type == 'store') {
                message.icon = S3Controller.getAwsS3SignedFileUrl(__MESSAGE_HEAD?.store?.logo_url);
            }else{
                message.icon = S3Controller.getAwsS3SignedFileUrl(__MESSAGE_HEAD?.user?.image_url);
            }
            message.image = S3Controller.getAwsS3SignedFileUrl(msg?.image_url);
            message.user_name = __MESSAGE_HEAD?.user?.first_name + " " + __MESSAGE_HEAD?.user?.last_name;
...
const fetchStoreMessages = async function (data) {
    ...
    try {
        ...
        const __MESSAGES = fetchMessages({ page, page_size, message_head_id: __MESSAGE_HEAD.id })
...
const sendMessageAndCreateNotification = async function (data, isSendNotification = false) {
    ...
    try {
        const { message_head_id, sender_type, message, image_url, is_user_read, is_store_read } = data;
        ...
        const __MESSAGE_HEAD = await MessageHead.update(
            { is_available_for_store: true, is_available_for_user: true,  status: 'active' },
            { 
                where: { id: message_head_id } ,
                returning: true,
                transaction: __SQL_TRANSACTION
        ...
        if(isSendNotification) {
            ...
            const storeOwner = await CommonController.getStoreOwnerUser(msgHead.store_id);

            let tokens = []
            if(sender_type == 'user') {
                const __IS_PUSH_NOTIFICATION_AVAILABLE = await CommonController.isOpenForNotification(storeOwner.id, true, 'message');
                if(__IS_PUSH_NOTIFICATION_AVAILABLE) {
                    const storeOwnerTokens = await CommonController.getUserTokens(storeOwner.id);
                    tokens = [...storeOwnerTokens];
                }
            
            }else{
                const __IS_PUSH_NOTIFICATION_AVAILABLE = await CommonController.isOpenForNotification(msgHead.user_id, false, 'message');
                if(__IS_PUSH_NOTIFICATION_AVAILABLE) {
                    const userTokens = await CommonController.getUserTokens(msgHead.user_id);
                    tokens = [...userTokens];
                }
            }
            ...
            const notificationData = {type: 'message', sender_type, message_head_id, store_id: msgHead.store_id}
            await SnsController.sendMutliPushNotifications(tokens,title , message, notificationData);
        }
...
const contactStoreMessages = async function(data, user_id) {
    ...
    try{
        ...
        let __MESSAGE_HEAD = await MessageHead.findOne({
            where: {user_id, store_id, offer_id: null, order_id: null, status: 'active'}
        })

        if(!__MESSAGE_HEAD) {
            __MESSAGE_HEAD = await MessageHead.create({
                user_id, store_id, is_available_for_store: false, is_available_for_user: true, is_store_completed: false, is_user_completed: false, status: 'active'
            })
        }

        const __STORE = await Store.findOne({where: {id: store_id}});
        const messages = await fetchMessages({page: 1, page_size: 10, message_head_id: __MESSAGE_HEAD.id});
...
module.exports = {
    createOfferMessageHeadsAndNotification, fetchUserMessageHeads, fetchUserMessages,
    createOrderMessageHead, userSendMessage, storeSendMessage, 
    fetchStoreMessageHeads, fetchStoreMessages, deleteStoreMessageHead,
    deleteUserMessageHead, contactStoreMessages, 
}

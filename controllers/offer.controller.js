  ...
  try {
    ...
    if (!offer.is_offer_for_store) {
      ...
      const __PRODUCTS = await Product.findAll({
        where: {
          store_id,
          id: { [Op.in]: offer_products.map((op) => op.product_id) },
          status: "active",
        },
      });
      ...
      const __PRODUCTS = await Product.findAll({
        where: { store_id: __OFFER.store_id, status: "active" },
      });
    ...
    await MessageController.createOfferMessageHeadsAndNotification(
      store_id,
      __OFFER.id,
      __OFFER.offer_name,
      __OFFER.image_url,
  ...
  try {
      ...
      include: [
        {
          ...
          required: true,
          where: { status: "active" },
          attributes: [["id", "store_id"], "store_name", "logo_url"],
          include: [
            {
              model: StoreAddress,
              required: false,
    ...
    return {
      ...
      offers: __OFFERS.data.map((o) => {
        ...
        const storeLogoImage = S3Controller.getAwsS3SignedFileUrl(
      ...
      include: [
        {
          ...
          required: false,
          where: { status: "active" },
          attributes: [["id", "offer_product_id"], "product_id", "status"],
          include: [
            {
              model: Product,
              required: false,
  ...
  try {
    ...
    const __OFFERED_PRODUCTS = await Product.findAll({
      ...
      include: [
        {
          model: OfferProduct,
          attributes: [["id", "offer_product_id"], "status"],
          where: { status: "active" },
        },
      ],
    });
...
const editStoreOffer = async function (data) {
  ...
  try {
    ...
    const expiredAt =
      offer.is_expired == true ? { expiredAt: moment().utc() } : {};

    const __OFFER = await Offer.findOne({
      where: { id: offer.offer_id, store_id },
    });
    ...
    await CommonController.bulkCreateUpdateDelete(
      OfferProduct,
      offer_products.map((oProduct) => {
        return { ...oProduct, offer_id: offer.offer_id };
      }),
      "offer_product_id",
  ...
  try {
    ...
    if (__OFFER.auto_created) {
      ...
      await Product.update(
        ...
        {
          where: {
            id: { [Op.in]: __OFFER_PRODUCTS.map((op) => op.product_id) },
          },
          transaction: __SQL_TRANSACTION,
        }
      );
...
const shop_listAllStoresOffers = async function (data) {
  ...
  try {
    ...
    const __STORES = await CommonController.getPaginationResult({
      ...
      page,
      page_size,
      order: [[Offer, sequelize.col("createdAt"), "DESC"]],
      as: "data",
    });
          ...
          include: [
            {
              ...
              where: {
                [Op.or]: [{ plan_type: "store" }, { plan_type: "combo" }],
                status: "active",
              },
            },
          ],
...
module.exports = {
  createStoreOffer,
  listStoreOffers,
  getStoreOfferDetails,
  listNonOfferProductsForStore,
  editStoreOffer,
  deleteStoreOffer,

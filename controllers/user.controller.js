...
const createUser = async function (data) {
    ...
    try {
        const { first_name, last_name, email, phone, phone_code, dob, has_store_access } = data;
        const status = "active";
        let user = null;

        const __USER = await User.findOne({where: {phone, phone_code, status, is_account_deleted: false}});

        if(has_store_access) {
            if(!__USER) {
                user = await User.create({ first_name, last_name, email, phone, phone_code, dob, status, has_store_access, is_account_deleted: false, user_balance: 0 }, { transaction: __SQL_TRANSACTION })
            }else{
                ...
                const store_access_registeration_detail = JSON.stringify({
                    first_name, last_name, email, createdAt: moment().utc().toDate()
                })

                user = __USER;
                await User.update({store_access_registeration_detail}, {where: {id: user.id}, transaction: __SQL_TRANSACTION})
        ...
        if(!has_store_access) {
            await SnsController.sendEmail(
                email, 
                "Welcome",
                createUser_Template(user.first_name + " " + user.last_name, has_store_access ),
        ...
        __SQL_TRANSACTION.afterCommit(() => {
            if(!__USER) {
                ['order', 'offer', 'message'].forEach(async (notification_type) => {
                    try{
                        await saveNotificationSetting({notification_type, is_for_store: false, is_enabled: true}, user.id )
                        has_store_access ? await saveNotificationSetting({notification_type, is_for_store: true, is_enabled: true}, user.id ) : ''
                    }catch(err){
                        console.log(err)
                    }
                })
...
const generateOTP = async function (data) {
    ...
    try {
        const user = await User.findOne({ where: { phone, phone_code, is_account_deleted: false, status: 'active' } });
        if (!user) {
            throw MESSAGES.USER_NOT_FOUND;
        }
        let otp = HELPERS.generateRandomNumber();
        if(otp.length <= 4) {
            if(otp.length == 1) {otp+="000"}
            if(otp.length == 2) {otp+="00"}
            if(otp.length == 3) {otp+="0"}
        }

        console.log({otp})
        const afterFiveMinutes = moment().utc().add(5, 'minutes');

        const __OTP__AVAILABLE = await UserOTP.findOne({ where: { user_id: user.id } });
        if (!__OTP__AVAILABLE) {
        ...
        const _sendOtp = await SnsController.sendOtp(user.phone, user.phone_code, otp);
        return otp;
    ...
    try {
        ...
        const userSession = await UserSession.create(
            { 
                ...
                expiredAt: moment().utc().add(parseInt(process.env.JWT_TOKEN_EXPIARATION), "days").toDate(), status: 'active' 
            },
            { transaction: __SQL_TRANSACTION }
        );
        ...
        const token = jwt.sign({
            data: {
                id: user.id, session_id: userSession.id, email: user.email, first_name: user.first_name, last_name: user.last_name, phone: user.phone
            },
        ...
        if(!user.has_store_access && user.store_access_registeration_detail) {
            const store_access_registeration_detail = JSON.parse(user.store_access_registeration_detail);
            const createdAt = moment(store_access_registeration_detail.createdAt).utc();
            ...
            if(currentDate.isBefore(createdAt)) {
                ...
                await SnsController.sendEmail(
                    ...
                    "Welcome",
                    createUser_Template(store_access_registeration_detail.first_name + " " + store_access_registeration_detail.last_name, true ),
                    process.env.ADMIN_EMAIL
        ...
        let isUserStoreOwner = $user && $user.is_store_owner ? $user.is_store_owner : false;
        ...
        const __USER = await User.findOne(
            {
                ...
                attributes: [
                    ['id', 'user_id'],
                    'uuid', 'first_name', 'last_name', 'nick_name' , 'email', 'phone', 'phone_code', 'image_url', 'has_store_access'
                ],
                include: [
                    {
                        model: UserAddress,
                        ...
                        attributes: [
                            ['id', 'user_address_id'],
                            'address_name', 'address_line_1', 'address_line_2', 'city', 'postal_code'
                        ],
                        include: [
                            {
                                model: State,
...
const logoutUser = async function(user_id) {
    try{
        await UserSession.update({status: 'deleted', expiredAt: moment().utc().toDate()}, {where: {user_id}})
 ...
 *  */
 const updateUserDetails = async function (data, user_id) {
    ...
    try{
        ...
        if(address) {
            const __ADDRESS = address.user_address_id ? await UserAddress.findOne({
                where: {id: address.user_address_id, user_id, status: 'active'}
            }) : null;
    
            const state_id = await CommonController.getStateId(address.country, address.state);
...
const deleteUser = async function(user_id) {
    try{
        ...
        const storeActiveOrders = await OrderController.listStoreOrders({
            page: 1, page_size: 10, order_by: 'order_id', order_type: "DESC", only_active_orders: true, from_date: null, to_date: null,
        },user_id)
        ...
        if(__STORES.length) {
            await Store.update({status: 'deleted'}, {where: {id: {[Op.in]: __STORES.map((s) => s.id)}}})
        }
...
const saveUserIdentityDetails = async function (data, user_id, is_verified) {
    ...
    try {
        const __PROOF_TYPE = await ProofType.findOne({ where: { id: proof_type_id, is_enabled: true, status: 'active' } });
        if (!__PROOF_TYPE) {
...
const saveFavouriteStore = async function (data, user_id) {
    ...
    try {
        ...
        const favouriteStore = await FavouriteStore.create({ store_id, user_id, status: 'active' });
...
const createFavouriteProduct = async function(product_id, user_id) {
    try{
        const __FAVOURITE_PRODUCT = await FavouriteProduct.create(
            {product_id, user_id, status: 'active'}
        )
...
const admin_listUsers = async function(data) {
    ...
    try{
        ...
        const order = order_by && order_type ? [[order_by, order_type]] : [['id', 'DESC']];
        ...
        const __USERS = await CommonController.getPaginationResult({
            Model: User, query, page, page_size, as: 'users',order
        })
...
const admin_blockUser = async function(data) {
    ...
    try{
        const {user_id} = data;
        const __IS_BLOCKED_USER = await BlockUser.findOne({where: {user_id, status: 'active'}});
...
module.exports = {
    createUser, generateOTP, verifyOTP, saveUserIdentityDetails,
    saveFavouriteStore, removeFavouriteStore, updateUserDetails,
    getUserDetials, createFavouriteProduct, removeFavouriteProduct, 
    allowUserStoreAccess, deleteUser, logoutUser,

    admin_listUsers, admin_userDetails, admin_blockUser, admin_removeBlockUser,

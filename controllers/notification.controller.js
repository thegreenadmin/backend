...
const listNotifications = async function (data, user_id) {
  ...
  try {
    ...
    const query = {
      where: {
        is_notification_for_store: is_notification_for_store == "true",
        user_id,
        status: "active",
      },
      attributes: {
    ...
    const notifications = __NOTIFICATIONS.data.map((notification) => {
      ...
      notification.store.logo = S3Controller.getAwsS3SignedFileUrl(
        notification.store.logo_url
      );
      notification.store.image = S3Controller.getAwsS3SignedFileUrl(
        notification.store.image_url
      );
...
const admin_listNotifications = async function (data) {
  ...
  try {
    ...
    const order =
      order_by && order_type
        ? [[order_by == "admin_notification_id" ? "id" : order_by, order_type]]
        : [["id", "DESC"]];
    ...
    await AdminNotification.update(
      { is_read: true },
      { where: { status: "active", is_read: false } }
    );

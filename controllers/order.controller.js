...
const iamhereNotification = async function(data) {
    try{
        ...
        const __USER = await User.findOne({where: { id: user_id, status: 'active' }});
        ...
        const Pdata = {
            store_id, order_id, type: 'order', sender_type:'user'
        }

        
        const userTokens = await CommonController.getUserTokens(user_id);
        await SNSController.sendMutliPushNotifications(userTokens, __NEW_ORDER_USER_NOTIFICATION.title, __NEW_ORDER_USER_NOTIFICATION.message, Pdata);
        await Notification.update({is_sent: true}, {where: {id: __NEW_ORDER_USER_NOTIFICATION.id}})
...
const createOrder = async function (data, user_id) {
    ...
    try {
        const { store_id, store_delivery_service_id, user_address_id, cart_items } = data;
        const __USER = await User.findOne({where: { id: user_id, status: 'active' }});
        const __ORDER_SERVICE_CHARGE = await CommonController.getOrderServiceCharge();
        ...
        const __STORE = await Store.findOne({
            where: { id: store_id, is_verified: true, is_enabled: true, status: 'active' }
        })
        ...
        const __OFFERS = await (await OfferController.shop_listStoreOffers(store_id, true)).offers;
        console.log("__OFFERS==",__OFFERS)
        let totalWeight = 0;
        ...
        const orderItems = __CART_ITEMS.map(item => {
            ...
            if (__STORE_OFFER) {
                ...
                for(let i=0; i<__OFFERS.length; i++) {
                    ...
                    const __IS_OFFER_PRODUCT = _offer?.offer_products?.find((op) => op.product_id == product.id);
            ...
            let maxDiscount = 0;          
             ...
             for(let i=0; i<offers.length; i++) {
                const discountOffset = CommonController.getCalculateOffset(product.selling_price, offers[i].offer_type, offers[i].offer_value)
            ...
            if (offer) {
                offerPrice = CommonController.getCalculatedPrice(product.selling_price, offer.offer_type, offer.offer_value);
            } else {
                offerPrice = CommonController.getCalculatedPrice(product.selling_price, product.discount_type, product.discount_value);
            }
            ...
            orderSubTotal += totalCharge;

           
            return {
                product_id: product.id,
                order_item_count: cart_items.find((ci) => ci.cart_item_id == cartItem.id).items_count,
        ...
        const __STORE_ADDRESS = await StoreAddress.findOne({ where: {store_id, status: 'active'}, include: [{model: State,include: [{model: Country}]}]})
        ...
        if(__USER_ADDRESS && __STORE_DELIVERY_SERVICE.delivery_service.delivery_service_name == 'Delivery') {
            const userAddressVerification  =await verifyAddress(__USER_ADDRESS.toJSON());
            if(!userAddressVerification.result) {throw "Address is non deliverable, please change to continue"}
        }

        let __DELIVERY_CHARGE = 0;
        const __DELIVERY_CHARGE_RES = (__STORE_DELIVERY_SERVICE && __STORE_ADDRESS) 
                                        ? await CommonController.getDeliveryServiceCharge(
                                            __STORE_DELIVERY_SERVICE.toJSON(), 
                                            __STORE_ADDRESS.postal_code, 
                                            (__USER_ADDRESS ? __USER_ADDRESS.postal_code : __STORE_ADDRESS.postal_code),
                                            totalWeight
                                        )
                                        : {result: true, rate: 0};
        ...
        let __TOTAL_TAX_CHARGED = CommonController.getCalculateOffset(orderSubTotal + __DELIVERY_CHARGE, __TAX.tax_type, __TAX.tax_value);
        const totalServiceCharge = CommonController.getCalculateOffset(orderSubTotal + __DELIVERY_CHARGE + __TOTAL_TAX_CHARGED, __ORDER_SERVICE_CHARGE.service_charge_type, __ORDER_SERVICE_CHARGE.service_charge_value);         
        const __ORDER = await Order.create(
            {
                ...
                total_amount: orderSubTotal + totalServiceCharge + __DELIVERY_CHARGE + __TOTAL_TAX_CHARGED,
        ...
        const __ORDER_ITEMS = await OrderItem.bulkCreate(
            orderItems.map(item => { return { ...item, order_id: __ORDER.id } }),
            { transaction: __SQL_TRANSACTION }
        )


        const __IS_CURB_SIDE_OR_IN_STORE_DELIVERY_SERVICE = (__STORE_DELIVERY_SERVICE.delivery_service.delivery_service_name.toLowerCase() == 'in store') || (__STORE_DELIVERY_SERVICE.delivery_service.delivery_service_name.toLowerCase() == 'curb side');
        ...
        const __ORDER_TRANSACTION = await TransactionController.createOrderTransaction(__ORDER, __USER, __SQL_TRANSACTION);

        // remove items from cart
        const __REMOVE_CART_ITEMS = await CartItem.update(
            {status: 'deleted'},
            {where: {id: {[Op.in] : cart_items.map((ci) => ci.cart_item_id)}}, transaction: __SQL_TRANSACTION}
        );
        await TransactionController.confirmOrderTransaction(__ORDER_TRANSACTION, __ORDER, __ORDER_ITEMS, store_id, __SQL_TRANSACTION)
        ...
        __SQL_TRANSACTION.afterCommit(async () => {
            try{
                ...
                const notifiableUsers = await CommonController.getStoreMangeOrderUserIds(store_id);
                ...
                const __ORDER_DETAILS = await orderDetails({order_id: __ORDER.id, store_id});
                SNSController.createAndSendCreateOrderEmails(__ORDER_DETAILS.order, [__USER.email], "Your Order Has Been Successfully Created!", `
                ...
                const storeEmails = await CommonController.getStoreMangeOrderUserEmails(store_id);
                SNSController.createAndSendCreateOrderEmails(__ORDER_DETAILS.order, storeEmails, "Your have received a new order", `
...
const listOrders = async function (data, user_id, forStore = false, order_id = null) {
    ...
    try {
        const { store_id, page, page_size, order_by, order_type, order_status_name, from_date, only_active_orders, to_date, is_show_products } = data;

        const stores =  store_id ? 
                            [{ store_id }] : 
                            forStore ? (await (await StoreController.getStoresWithManageOrderPermissions(user_id)).stores) : [];
        
        const order = (order_by && order_type) ? [[sequelize.col(order_by), order_type]] : [['id', 'DESC']];

        const whereStores = stores.length ? {store_id: {[Op.in]: stores.map(s => s.store_id)}} : (forStore ? {store_id: null} : {});

        const whereOrderStatus = order_status_name ? [{ order_status_id: await (await OrderStatus.findAll({where: {order_status_name}})).map(os => os.id) }] : [];

        const nonActiveOrderStatusIds = await (await OrderStatus.findAll({where: {order_status_name: {[Op.in] : ['completed','cancelled', 'returned']}}})).map(os => os.id);

        const notWhereOrderStatuses = only_active_orders != null 
                                        ? [{order_status_id: {[only_active_orders ? Op.notIn : Op.in] : nonActiveOrderStatusIds}}]                                  
                                        : []
        ...
        const whereDate = (from_date && to_date) ? {
            [Op.and]: [
                {
                    createdAt: { [Op.gte]: moment(from_date).utc().subtract(1, "days").toDate() },
                },
                {
                    updatedAt: { [Op.lte]: moment(to_date).utc().subtract(1, "days").toDate() }
                }
            ]
        } : {};
        ...
        const query = {
            ...
            include: [
                ...includeOrderTransaction,
                {
                    ...
                    attributes: [
                        ['id', 'store_id'],
                        'logo_url', 'store_name', 'is_verified', 'is_enabled', 'image_url'
                    ],
                },
                {
                    model: OrderHistory,
                    attributes: [
                        ['id', 'order_history_id'],
                        'order_status_id', 'createdAt', 'updatedAt'
                    ],
...
const listStoreOrders = async function (data, user_id) {
    ...
    try {
        let { store_id, page, page_size, order_by, order_type, only_active_orders, order_statuses, from_date, to_date } = data;
        ...
        if (order_statuses &&  order_statuses.length) {
            ...
            const filterOrderStatusNames = order_statuses.filter(os => !os.as).map( os => os.order_status_name)
            if(filterOrderStatusNames) {
                return await listOrders({...data, order_status_name: {[Op.in]: filterOrderStatusNames}},user_id, true)
            }
...
const listStoreActiveOrders = async function(data, user_id) {
    ...
    try{
        ...
        const order_statuses = await OrderStatus.findAll({
            where: {order_status_name: {[Op.notIn]: ['created','delivered', 'cancelled']}}
        })
        ...
        if (order_statuses && order_statuses.length) {
            ...
            for (let i = 0; i < order_statuses.length; i++) {
                ...
                const __ORDERS = await listOrders({ ...data, only_active_orders: true, order_status_id: os.id }, user_id, true)
                const __ORDER_STATUS = await OrderStatus.findOne({where: {id: os.id}})

                const key = orders[__ORDER_STATUS.order_status_name] ? __ORDER_STATUS.order_status_name : 'received';
...
const listUserOrders = async function(data, user_id) {
    ...
    try {
        const { store_id, page, page_size, order_by, order_type, only_active_orders, order_statuses, from_date, to_date } = data;

        if (order_statuses.length) {
            const filterOrderStatusNames = order_statuses.filter(os => !os.as).map( os => os.order_status_name)
            if(filterOrderStatusNames) {
                return await listOrders({...data, order_status_name: {[Op.in]: filterOrderStatusNames}},user_id, false)
            }
        } else {
    ...
    try{
        ...
        const __ORDER = await Order.findOne(
            {
                ...
                include: [
                    ...
                    },
                    {
                        ...
                        required: true,
                        attributes: [
                            ['id', 'order_history_id'], 'is_created_by_store',
                            'order_status_id', 'is_current_status', 'createdAt', 'updatedAt'
                        ],
                        include: [
                            {
                                model: OrderStatus,
                                ...
                                attributes: [
                                    ['id', 'order_status_id'],
                                    'order_status_name', 'order_state_number'
                                ]
                            }
                        ]
                    },
        ...
        order.order_items = order.order_items.map(item => {
            ...
            item.enable_return_button = item.product.is_product_returnable ? 
                item.return_order_items.length ? false :
                moment(order.createdAt).utc().add('days', item.product.return_days_count) > moment().utc()
            : false;
            ...
            item.offer_price = item.order_item_price - CommonController.getCalculateOffset(item.order_item_price, item.discount_type, item.discount_value);

            return item;
        })
        
        order.order_items.sort((a, b) => parseInt(a.order_item_id) - parseInt(b.order_item_id));
     ...
     const sentNotification=  await Notification.findOne({where: {order_id: order_id,is_sent:true}})
...
const isStoreOrder = async function(store_id, order_id) {
    try{
        const __ORDER = await Order.findOne(
            ...
            include: [{model: Store},{model: OrderDeliveryAddress,include: [{model: State, include: [{model: Country}]}]}]},
...
const isUserOrder = async function(user_id, store_id, order_id) {
    try{
        const __ORDER = await Order.findOne({where: {store_id, user_id, id: order_id, status: 'active'},include: [{model: Store}, {model: OrderDeliveryAddress, include: [{model: State, include: [{model: Country}]}]}]});
        if(!__ORDER) {
            throw "Order not found";
        }
        return __ORDER;
...
const isOrderHistoryExists = async function(order_id, orderStatus = {}, throwErr = "") {
    try{
...
const getOrderStatus = async function(order_status_name) {
    try{
        const __ORDER_STATUS = await OrderStatus.findOne({where: {order_status_name, status: 'active'}});
        if(!__ORDER_STATUS) {
...
const updateOrder = async function(order_id, order_status_id, is_created_by_store, __SQL_TRANSACTION) {
    try{
...
const createPendingOrder = async function(data) {
    ...
    try{
        ...
        const __ORDER_HISTORY = await isOrderHistoryExists(order_id, __PENDING_ORDER_STATUS);
...
const confirmOrder = async function(data) {
    ...
    try{
        ...
        const __ORDER_HISTORY = await isOrderHistoryExists(order_id, __CONFIRM_ORDER_STATUS);
        ...
        const __ORDER_ITEMS = await OrderItem.findAll({
            where: { order_id, id: {[Op.in]: order_items.map((oi) => oi.order_item_id)}},
            include: [{model: Product}]
        })

        
        if(!__ORDER_HISTORY) {
            const __NEW_ORDER_HISTORY = await updateOrder(order_id, __CONFIRM_ORDER_STATUS.id, true, __SQL_TRANSACTION)
...
const createCancelOrderRequest = async function(data, user_id) {
    ...
    try{
        ...
        return cancelOrder({store_id, order_id, order_items : __ORDER_ITEMS.map(item => {return {order_item_id: item.id}})})
    }catch(err) {
        throw err;
    }
}
...
const cancelOrder = async function(data) {
    ...
    try{
        ...
        const __NEW_ORDER_HISTORY = await updateOrder(order_id, __CANCELLED_ORDER_STATUS.id, true, __SQL_TRANSACTION)
        await TransactionController.createCancelOrderTransaction(order_id, __SQL_TRANSACTION);
...
const createReadyPickupOrder = async function(data) {
    ...
    try{
        ...
        const __READY_PICKUP_ORDER_STATUS = await getOrderStatus('ready for pickup', true, false);
        const __SHIPPED_ORDER_HISTORY = await isOrderHistoryExists(order_id, __READY_PICKUP_ORDER_STATUS);
        ...
        if(!__SHIPPED_ORDER_HISTORY) {
            const __NEW_ORDER_HISTORY = await updateOrder(order_id, __READY_PICKUP_ORDER_STATUS.id, true, __SQL_TRANSACTION)
        }
        ...
        __SQL_TRANSACTION.afterCommit(async () => {
            try{
                const __STORE = await Store.findOne({where: {id: __ORDER.store_id}})
                const __USER = await User.findOne({where: {id: __ORDER.user_id}})
                SNSController.sendOrderStatusEmail(
                    __ORDER,
                    __NOTIFICATION_ITEMS,
...
const createUserReadyPickupOrder = async function(data, user_id) {
    ...
    try{
        ...
        const __USER_READY_PICKUP_HISTORY = await isOrderHistoryExists(order_id, __USER_READY_PICKUP);

        if(!__USER_READY_PICKUP_HISTORY) {
            const __NEW_ORDER_HISTORY = await updateOrder(order_id, __USER_READY_PICKUP.id, false, __SQL_TRANSACTION)
            await OrderHistory.update({is_created_by_store: true}, {where: {id: __NEW_ORDER_HISTORY.id}})
        }


        const __NOTIFICATION_ITEMS = await OrderItem.findAll({
            where: {
                order_id, order_item_status: 'in progress'
            },
            include: [
                {
                    model: Product
...
const shippedOrder = async function(data) {
    ...
    try{
        ...
        const __SHIPPED_ORDER_HISTORY = await isOrderHistoryExists(order_id, __SHIPPED_ORDER_STATUS);
        ...
        if(!__SHIPPED_ORDER_HISTORY) {
            const __NEW_ORDER_HISTORY = await updateOrder(order_id, __SHIPPED_ORDER_STATUS.id, true, __SQL_TRANSACTION)
        }
...
const deliverOrder = async function(data) {
    ...
    try{
        ...
        const __DELIVERED_ORDER_HISTORY = await isOrderHistoryExists(order_id, __DELIVERED_ORDER_STATUS);

        let currentStoreBalance = await CommonController.getStoreBalance(store_id);

        if(!__DELIVERED_ORDER_HISTORY) {
            const __NEW_ORDER_HISTORY = await updateOrder(order_id, __DELIVERED_ORDER_STATUS.id, true, __SQL_TRANSACTION)
            currentStoreBalance = await TransactionController.createStoreOrderTransctionAndReturnBalance(store_id, order_id, __SQL_TRANSACTION);
        }
        ...
        const __CANCELLABLE_ITEMS = await OrderItem.findAll({
            where: { order_id, id: {[Op.notIn]: order_items.map((oi) => oi.order_item_id)}},
            include: [{model: Product}]
        })
        ...
        if(__CANCELLABLE_ITEMS.length) {
            ...
            await TransactionController.createAndConfirmOrderItemCancelTransaction(order_id, __CANCELLABLE_ITEMS.map(ci => {return {order_item_id: ci.id}}), currentStoreBalance, __SQL_TRANSACTION);
        }

        __SQL_TRANSACTION.afterCommit(async () => {
            try{
                const __USER = await User.findOne({where: {id: __ORDER.user_id}})
                const __STORE_EMAILS = await CommonController.getStoreMangeOrderUserEmails(store_id);
                SNSController.sendOrderStatusEmail(
                    __ORDER,
                    __NOTIFICATION_ITEMS,
            ...
            include: [
                {
                    model: OrderItem,
                    required: true,
                    where: {product_id, order_item_status: {
                        [Op.in] : ['completed', 'return request', 'return confirmed', 'returned']
                    }, status: 'active'}
                },
            ]
        })
...
const createProductsReview = async function(data, user_id) {
    ...
    try{
        ...
        const __IS_ORDER_COMPLETED = await isProductOrderCompleted(order_id, product_id, user_id);
        
        const __REVIEW = await ProductReview.create({order_id, product_id, user_id, review, rating, status: 'active'});
...
// return order
// return order
const isOrderItemsCompleted = async function( user_id, store_id, order_id, order_items) {
    // order_items : {order_item_id}
    try{
        const __DELIVERED_ORDER_STATUS = await getOrderStatus('completed');
        const __ORDER = await Order.findOne({
            ...
            include: [
                {
                    model: OrderItem,
                    required: true,
                    where: {id : {[Op.in]: order_items.map(item => item.order_item_id)}, status: 'completed', status: 'active'}
                },
                {
                    model: OrderHistory,
                    required: false,
                    where: {order_status_id: __DELIVERED_ORDER_STATUS.id, status: 'active'}
                }
            ]
        })
...
const createReturnOrderItemsRequest = async function(data, user_id) {
    ...
    try{
        const {store_id, order_id, order_items} = data;
        const __ORDER = await isOrderItemsCompleted(user_id, store_id, order_id, order_items);
        ...
        const __SELECTED_DELIVERED_ORDER_ITEMS = await OrderItem.findAll({
            where: {id: {[Op.in]: order_items.map(item => item.order_item_id)}, order_item_status: 'completed', status: 'active'},
            include: [
                {
                    model: Product,
                    required: false,
        ...
        const returnOrderItems = __SELECTED_DELIVERED_ORDER_ITEMS.map(item => {
            ...
            const returnOrderItem = order_items.find(o => o.order_item_id == item.id);
            const returnOrderItemCount = returnOrderItem.return_items_count;

            const totalProductPrice = (orderItem.order_item_price * orderItem.order_item_count);
            const totalDiscount = CommonController.getCalculateOffset(totalProductPrice, orderItem.discount_type, orderItem.discount_value);
        ...
        const __RETURN_ORDER_REQUEST_STATUS = await OrderStatus.findOne({where: {order_status_name: 'return request', status: 'active'}});
        const __RETURN_REQUEST_HISTORY = await isOrderHistoryExists(order_id, __RETURN_ORDER_REQUEST_STATUS);

        if(!__RETURN_REQUEST_HISTORY) {
            const __NEW_ORDER_HISTORY = await updateOrder(order_id, __RETURN_ORDER_REQUEST_STATUS.id, false, __SQL_TRANSACTION)
        }
...
const confirmReturnOrderItemRequest = async function(data) {
    ...
    try{
        ...
        const __CONFIRM_RETURN_ORDER_STATUS = await getOrderStatus('return confirmed');
        const __CONFIRM_RETURN_ORDER_HISTORY = await isOrderHistoryExists(order_id, __CONFIRM_RETURN_ORDER_STATUS)

        if(!__CONFIRM_RETURN_ORDER_HISTORY) {
            const __NEW_ORDER_HISTORY = await updateOrder(order_id, __CONFIRM_RETURN_ORDER_STATUS.id , true, __SQL_TRANSACTION)
        }
...
const cancelReturnOrderRequest = async function(data, user_id = null) {
    ...
    try{
        ...
        const __CANCEL_RETURN_ORDER_STATUS = await getOrderStatus('return cancelled');
        const __CANCEL_RETURN_ORDER_HISTORY = await isOrderHistoryExists(order_id, __CANCEL_RETURN_ORDER_STATUS, "Return is already cancelled")

        const __COMPLETED_RETURN_ORDER_STATUS = await getOrderStatus('returned');
        const __COMPLETED_RETURN_ORDER_HISTORY = await isOrderHistoryExists(order_id, __COMPLETED_RETURN_ORDER_STATUS, "Return is completed, and can not be reversed")

        if(!__CANCEL_RETURN_ORDER_HISTORY) {
            const __NEW_ORDER_HISTORY = await updateOrder(order_id, __CANCEL_RETURN_ORDER_STATUS.id, true, __SQL_TRANSACTION)
        }
...
const completeReturnRequest = async function(data)  {
    ...
    try{
        ...
        const __COMPLETE_RETURN_ORDER_HISTORY = await isOrderHistoryExists(order_id, __COMPLETE_RETURN_ORDER_STATUS)

        // update order item status
        const __UPDATE_ORDER_ITEM_STATUS = await OrderItem.update({
            order_item_status: 'returned'
        }, {
            where: {order_id, id: {[Op.in]: order_items.map(item => item.order_item_id)}},
            transaction: __SQL_TRANSACTION
        })
        ...
        if(order_items.length) {
            const __RETURN_ITEM_TRANSACTION = await TransactionController.createReturnOrderTransction(order_id, __ORDER_ITEMS, __SQL_TRANSACTION);
        }

        if(!__COMPLETE_RETURN_ORDER_HISTORY) {
            const __NEW_ORDER_HISTORY = await updateOrder(order_id, __COMPLETE_RETURN_ORDER_STATUS.id, true, __SQL_TRANSACTION)
        }
        ...
        const __NON_RETURNABLE_ITEMS = await OrderItem.findAll({
            where: {order_id, id: {[Op.notIn]: __ORDER_ITEMS.map(item => item.order_item_id)}},
            include: [
                {
                    model: ReturnOrderItem
                },
        ...
        await OrderItem.update({
            ...
            where: {id: {[Op.in]: __NON_RETURNABLE_ITEMS.map(roi => roi.order_item_id)}},
            transaction: __SQL_TRANSACTION
        })
...
const admin_listOrders = async function(data) {
    ...
    // order_statuses = [{order_status_name}]
    try{
        const { order_id, store_id, page, page_size, order_by, order_type, order_statuses, from_date, to_date} = data;
        const filterOrderStatusNames = order_statuses.filter(os => !os.as).map( os => os.order_status_name)
        ...
        if(filterOrderStatusNames.length) {
            ...
            }, null, false, order_id == "" || !order_id ? null : order_id)
        }
        else {
            return await listOrders({
                ...data,
...
const admin_saveOrderServiceCharge = async function(data) {
    ...
    try{
        ...
        const __ORDER_SERVICE_CHARGE = await OrderServiceCharge.findOne({where: {status: 'active'}})

        if(!__ORDER_SERVICE_CHARGE) {
            await OrderServiceCharge.create({service_charge_type, service_charge_value, status: 'active'})
        }else{
...
module.exports = {
    iamhereNotification,createOrder, listStoreOrders, listUserOrders, orderDetails, 
    createCancelOrderRequest, cancelOrder, confirmOrder, createPendingOrder,
    listOrderStatuses, shippedOrder, deliverOrder, listStoreActiveOrders, 
    createProductsReview, isUserProductReviewExists, createReturnOrderItemsRequest,
    confirmReturnOrderItemRequest, cancelReturnOrderRequest, completeReturnRequest,
    getOrderUserProofDetails, createReadyPickupOrder, createUserReadyPickupOrder,


    admin_listOrders, admin_saveOrderServiceCharge, 
}
...
const upsert = async function(Model, obj, whereObj, __SQL_TRANSACTION) {
    try{
        const row = await Model.findOne({where: whereObj, transaction: __SQL_TRANSACTION});
        if(!row) {
...
const getControllerByControllerKey = async function (controller_key, shouldThrowError = false) {
    try{
        const controller = await Controller.findOne({where: {controller_key, status: 'active'}});
        if(!controller) {
            if(shouldThrowError) {
...
const getControllerById = async function (controller_id, shouldThrowError = false) {
    try{
        const controller = await Controller.findOne({where: {id: controller_id, status: 'active'}});
        if(!controller) {
            if(shouldThrowError) {
                throw MESSAGES.CONTROLLER_NOT_FOUND
            }
...
const checkStoreStatus = async function (store_id) {
    const store = await Store.findOne({where: {id: store_id, status: 'active'}});
    ...
    const user = await User.findOne(
        {
            ...
            include: [
                {
                    model: StoreUser,
                    where: {user_id: user_id, is_verified: true, store_id, status: 'active'},
                    required: true   
                }
            ]
        }
 ...
 * @description check wheter the store user has permission to particular controller or not
 *  */
const getPaginationResult = async function({Model, query, page, page_size, order, as}) {
 ...
 * @description check wheter the store user has permission to particular controller or not
 *  */
const bulkCreateUpdateDelete = async function(Model, dataArr = [], key, whereKeys = {}, __SQL_TRANSACTION) {
    try{
        const bulkCreateObj = [];
        const bulkDeleteIds = [];
...
const getStateId = async function(country, state) {
    try{
        const __COUNTRY = await Country.findOne({where: {country_name: {[Op.iLike]: country}}});
        if(!__COUNTRY) {throw "Country not found"}

        let __STATE = await State.findOne({where: {country_id: __COUNTRY.id, state_name: {[Op.iLike]: state}}})

        if(!__STATE) {
            state = getStateName(state, __COUNTRY.abbrevation);
            __STATE = await State.findOne({where: {country_id: __COUNTRY.id, state_name: {[Op.iLike]: state}}})
        }

        if(!__STATE) {throw "State not found"}
...
const getOrderServiceCharge = async function() {
    try{
        let __ORDER_COMMISION = await OrderServiceCharge.findOne({where: {status: 'active'}});
...
const getDeliveryServiceCharge = async function(store_delivery_service, store_postal_code, user_postal_code, weight) {
    try{
        const __DELIVERY_SERVICE = store_delivery_service.delivery_service;
        if(!(__DELIVERY_SERVICE.delivery_service_name.toLowerCase() == 'delivery')) {
            return {
                result: true,
                rate: 0
            };
            ...
            return await calculateShippingCost(store_postal_code, user_postal_code, weight);
        }
    }catch(err) {
        throw err;
    }
...
const getTaxRate = async function(from_address, destination_address, store_id, totalProductPrice, delivery_charge, service_charge = 0) {
    try{
        const __TAX = await Tax.findOne({where: {status: 'active'}});
        ...
        return {
            tax_type: 'percentage',
            tax_value: ((parseFloat(calculation.tax_amount_exclusive) + parseFloat(calculation.tax_amount_inclusive)) * 100)/parseFloat(calculation.amount_total)
        };
            ...
            include: [
                {
                    ...
                    include: [
                        {
                            model: Permission,
                            where: {controller_id: manageOrderController.id, status: 'active'}
                        }
                    ]
                }
            ]
...
const updateStoreBalance = async function(store_id, store_balance, __SQL_TRANSACTION) {
    try{
        return await Store.update({store_balance}, {where: {id: store_id}, transaction: __SQL_TRANSACTION})
    }catch(err) {
        throw err;
    }
}

const updateUserBalance = async function(user_id, user_balance, __SQL_TRANSACTION) {
    try{
        return await User.update({user_balance}, {where: {id: user_id}, transaction: __SQL_TRANSACTION})
    }catch(err) {
        throw err;
    }
}
...
const getStripeTransferPaymentService = async function() {
    try{
        const __PAYMENT_SERVICE = await PaymentService.findOne({
            where: {payment_service_name: {[Op.iLike]: "Stripe transfer"}, is_enabled: true, status: 'active'}
        });
...
const isOpenForNotification = async function(user_id, is_for_store = false, notification_type) {
    try{
        const __NOTIFICATION_SETTING = await NotificationSetting.findOne({
            where: {
                user_id, is_for_store, notification_type, status: 'active'
            }
        })
...
module.exports = { 
    upsert, getControllerByControllerKey, getControllerById, isUserVerified, 
    getPaginationResult, checkStoreStatus, getCalculatedPrice,
    bulkCreateUpdateDelete, isUserBelongsToStore, getFiltersFromArray,
    getOrderServiceCharge, getStoreBalance, getStoreServiceCharge,
    getDeliveryServiceCharge, getTaxRate, getCalculateOffset,
    getUserTokens, getStoreMangeOrderUserTokens, getStoreOwnerUser, 
    getStripePaymentService, getUserCurrentBalance, getStateId, 
    getStripeTransferPaymentService, getStoreMangeOrderUserIds, isOpenForNotification,
    getStoreMangeOrderUserEmails, getFullResult, updateStoreBalance, updateUserBalance,


    getPageHtml, getStateAbbreviationByName, createXLSXfile, getDefaultTax, 
    updateDefaultTax,

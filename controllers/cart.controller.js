    ...
    try{
        ...
        if(__PRODUCT.store_id) {
            ...
            if(__NON_ACTIVE_CART_ITEM.length) {
                await CartItem.update({status: 'deleted'},{where: {id: {[Op.in]: __NON_ACTIVE_CART_ITEM.map((ci) => ci.id)}}})
            } 
        }
        ...
        if(__PRODUCT_ITEM_ALREADY_EXISTS) {
            await CartItem.update({items_count: __PRODUCT_ITEM_ALREADY_EXISTS.items_count + items_count}, {where: {id: __PRODUCT_ITEM_ALREADY_EXISTS.id}});
            return {cart_item_id: __PRODUCT_ITEM_ALREADY_EXISTS.id}
        }

        const __CART_ITEM = await CartItem.create({product_id, items_count, user_id, status: 'active'});
    ...
    try{
        ...
        const __STORE_DELIVERY_SERVICE = store_delivery_service_id 
                                            ?   await StoreDeliveryService.findOne({
                                                    where: { id: store_delivery_service_id, store_id, is_enabled: true, status: 'active'},
                                                    include: [{model: DeliveryService, is_enabled: true,status: 'active'}]
                                                })
                                            : null;
        ...
        console.log("cart __OFFERS==",__OFFERS)
        let subTotal = 0.0
        ...
        const cartItems = __CART_ITEMS.map(item => {
            ...
            const productImage = S3Controller.getAwsS3SignedFileUrl(cartItem.product?.product_images[0]?.image_url);
            ...
            if(__STORE_OFFER) {            
                ...
                for(let i=0; i<__OFFERS.length; i++) {
                    ...
                    const __IS_OFFER_PRODUCT = _offer?.offer_products?.find((op) => op.product_id == product.product_id);
            ...
            if(offer) {
                ...
                itemDiscountOffset = CommonController.getCalculateOffset(product.selling_price, offer.offer_type, offer.offer_value)
                console.log("itemDiscountOffset 1",itemDiscountOffset)
            }else{
            
                itemDiscountOffset = CommonController.getCalculateOffset(product.selling_price, product.discount_type, product.discount_value)
        ...
        if(__USER_ADDRESS) {
            isValidAddress = await (await verifyAddress(__USER_ADDRESS.toJSON())).result;
        }
        ...
        let totalTax = CommonController.getCalculateOffset(subTotal - totalDiscount + __DELIVERY_CHARGE, __TAX.tax_type, __TAX.tax_value)
        console.log("cart section start")
        console.log("totalTax==",totalTax)
        console.log("subTotal==",subTotal,"__DELIVERY_CHARGE=",__DELIVERY_CHARGE,"totalTax=",totalTax,"__ORDER_SERVICE_CHARGE.service_charge_type=",__ORDER_SERVICE_CHARGE.service_charge_type,"__ORDER_SERVICE_CHARGE.service_charge_value=",__ORDER_SERVICE_CHARGE.service_charge_value)
        let totalServiceCharge = CommonController.getCalculateOffset(subTotal - totalDiscount + __DELIVERY_CHARGE + totalTax, __ORDER_SERVICE_CHARGE.service_charge_type, __ORDER_SERVICE_CHARGE.service_charge_value)
        console.log("totalServiceCharge==",totalServiceCharge)                 
        console.log("cart section end")
        const totalAmount = subTotal - totalDiscount + totalTax + __DELIVERY_CHARGE + totalServiceCharge;
...
module.exports = {
    createCartItem, listStoreCartItems, updateStoreCartItem,
    deleteStoreCartItem,  currentlyActiveCart, 
}
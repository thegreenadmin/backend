...
const getStripeUser = async function(user_id, throwErr = true) {
    try{
        const __STRIPE_USER = await UserStripe.findOne({where: {user_id, status: 'active'}})
...
const createStripeUser = async function(data, user_id) {
    ...
    try{
        ...
        const __USER_ADDRESS = user_address_id 
            ? await UserAddress.findOne({where: {id: user_address_id, user_id, status: 'active'}})
            : null;
        
        const __STATE = user_address_id ? await State.findOne({where: {id: __USER_ADDRESS.state_id}}) : null
        const __COUNTRY = user_address_id ? await Country.findOne({where: {id: __STATE.country_id}}) : null
...
const createUserStripeCard = async function(data, user_id) {
    ...
    try{
        ...
        if(!__USER_STRIPE) {
            ...
            await createStripeUser({user_address_id: __USER_ADDRESS ? __USER_ADDRESS.id : null}, user_id);
...
const listUserCards = async function(user_id) {
    try{
        ...
        const cards = await stripe.customers.listSources(__USER_STRIPE.stripe_customer_id)

        return {cards: __USER_STRIPE_CARDS.map(usc => {
            usc = usc.toJSON();
...
const deleteUserStripeCard = async function(data, user_id) {
    ...
    try{
        ...
        const __USER_STRIPE_CARD = await UserStripeCard.findOne({
            where: {id: user_stripe_card_id, user_stripe_id: __USER_STRIPE.id, status: 'active'}
        })
...
const userWalletRechargeWithCard = async function(data, user_id, user_wallet_auto_charge_id = null) {
    // data = {user_stripe_card_id, amount}

    try{
        const {user_stripe_card_id, amount} = data;
        const __STRIPE_PAYMENT_SERVICE = await CommonController.getStripePaymentService();
        ...
        if(paymentIntent) {
            ...
            if(paymentIntent.status != 'succeeded') {
                ...
                const __IS_TRANSACTION_ALREADY_UPDATED = await UserWalletTransactions.findOne({
                    where: {
                        transaction_id: __TRANSACTION.id
                    }
                })
                ...
                if(!__IS_TRANSACTION_ALREADY_UPDATED){
                    ...
                    const __SUCCESS_TRANSACTION_HISTORY = await TransactionHistory.create(
                        {transaction_id: __TRANSACTION.id, transaction_status: 'success', status: 'active'}
                    );
    
                    const __USER_BALANCE = await CommonController.getUserCurrentBalance(user_id);

                    const __USER_WALLET_TRANSACTION = await UserWalletTransactions.create({
                        transaction_id: __TRANSACTION.id,
                        user_id,
                        user_stripe_card_id : user_stripe_card_id ? user_stripe_card_id : null,
                        user_wallet_auto_charge_id : null,
                        net_balance: __USER_BALANCE + parseFloat(amount),
                        status: 'active'
                    })
    
                    await User.update({user_balance: __USER_BALANCE + parseFloat(amount)}, {where: {id: user_id}})
                }
            } 
        }

        return {transaction_id: __TRANSACTION.id, amount, status: paymentIntent.status}
    }catch(err) {
        throw err;
    }
}
    ...
    try{
        ...
        if(paymentIntent) {
            ...
            if(paymentIntent.status != 'succeeded') {
                ...
                const __IS_TRANSACTION_ALREADY_UPDATED = await StoreWalletTransaction.findOne({
                    where: {
                        transaction_id: __TRANSACTION.id
                    }
                })
                ...
                if(!__IS_TRANSACTION_ALREADY_UPDATED){
                    ...
                    await Store.update({store_balance: __STORE_BALANCE + parseFloat(amount)}, {where: {id: store_id}})
                }
            } 
        }
...
const createUserWalletRechargeWithApplePayOrGooglePay = async function(data, user_id) {
    ...
    try{
        ...
        const __NON_REGISTERED_BANKS = banks.data.filter((c) => {
            ...
            __USER_STRIPE_BANKS.forEach(usb => {
                isBankExists = !isBankExists ? usb.stripe_bank_id == c.id : isBankExists
            })
            return !isBankExists;
        })
        ...
        return {banks: __USER_STRIPE_BANKS.map(usc => {
            usc = usc.toJSON();
...
const deleteUserBankAccount = async function(data, user_id) {
    ...
    try{
        ...
        const __USER_STRIPE_BANK = await UserStripeBank.findOne({
            where: {id: user_stripe_bank_id, user_stripe_id: __USER_STRIPE.id, status: 'active'}
        })
...
const createStoreStripeAccount = async function(data, ip) {
    ...
    try{
        ...
        const __STRIPE_ACCOUNT_EXISTS = await getStoreStripeAccount(store_id, false);

        if(__STRIPE_ACCOUNT_EXISTS) {
            throw "Store stripe account already exists"
        }

        const __STORE = await Store.findOne({where: {id: store_id, status: 'active'}})
        ...
        return {store_stripe_account_id: __STORE_STRIPE_ACCOUNT.id, link: accountLink.url}
...
const createStoreStripeBankAccount = async function(data, ip) {
    ...
    try{
        ...
        if(!__STORE_STRIPE_ACCOUNT) {
            ...
            await createStoreStripeAccount({store_id, store_address_id: __STORE_ADDRESS.id}, ip)
...
const createStripePayout = async function(data, user_id) {
    ...
    try{
        ...
        const __STORE_SERVICE_CHARGE = await CommonController.getStoreServiceCharge(store_id);

        const storeBalance = await CommonController.getStoreBalance(store_id);
        
        const totalAmount = amount;
        const newStoreBalance = (storeBalance - totalAmount).toFixed(2) == 0 ? 0 : (storeBalance - totalAmount);
        const stripeBalance = await stripe.balance.retrieve();

        const __STRIPE_PAYOUT_PAYMENT_SERVICE = await CommonController.getStripeTransferPaymentService();

        if(totalAmount > storeBalance) {
            throw "Not enough balance"
        }

        if(!stripeBalance || !stripeBalance.available.length || (totalAmount*100 > stripeBalance.available[0].amount)) {
        ...
        await Store.update({store_balance: newStoreBalance}, {where: {id: store_id}});
        ...
        if(transfer) {
            await StorePayout.update({stripe_transfer_id: transfer.id}, {where: {id: __STORE_PAYOUT.id}})
        }
        ...
        if(payout) {
            await Transaction.update({stripe_payout_transaction_id: payout.id},{where: {id: __TRANSACTION.id}})
            await StorePayout.update({stripe_payout_id: payout.id}, {where: {id: __STORE_PAYOUT.id}})
        }

        return {store_payout_id: __STORE_PAYOUT.id}
    }catch(err) {
        if(storeId, userStripeBankId, store_wallet_transaction_id, transaction_id) {
            try{
                returnStripePayout({
                    user_id, store_id: data.store_id, store_wallet_transaction_id, user_stripe_bank_id: userStripeBankId, transaction_id
                })
            }catch(err) {
                // logger.log(err)
            }
...
const returnStripePayout = async function(data) {
    ...
    try{
        ...
        const {user_id, store_id, store_wallet_transaction_id, user_stripe_bank_id, transaction_id} = data;
        ...
        const __USER_STRIPE = await UserStripe.findOne({where: {user_id, status: 'active'}});
        ...
        const storeBalance = await CommonController.getStoreBalance(store_id);
        const newStoreBalance = storeBalance + __STORE_PAYOUT.total_transaction_amount;

        const __CREDIT_STORE_WALLET_TRANASACTION = await StoreWalletTransaction.create({
            store_id,
        ...
        await StorePayout.update({stripe_transfer_id: reversal.id}, {where: {id: __CREDIT_STORE_PAYOUT.id}})
        await StorePayout.update({is_reversed: true}, {where: {id: __STORE_PAYOUT.id}})
    ...
    try{
        ...
        switch (event.type) {
            case 'payment_intent.succeeded': {
                ...
                const {transaction_id, user_id, user_stripe_card_id, NODE_ENV, user_wallet_auto_charge_id, store_id} = event.data.object.metadata;

                if(NODE_ENV != process.env.NODE_ENV) {
                    return;
                }

                const __UPDATE_TRANSACTION = await Transaction.update({stripe_transaction_id: event.data.id}, {where: {id: transaction_id}});
                ...
                if(store_id) {
                    ...
                    if(!__IS_TRANSACTION_ALREADY_UPDATED){
                        ...
                        const __SUCCESS_TRANSACTION_HISTORY = await TransactionHistory.create(
                            {transaction_id: transaction_id, transaction_status: 'success', status: 'active'}
                        );
                    ...
                    const __USER_WALLET_TRANSACTION = await UserWalletTransactions.create({
                        ...
                        user_id,
                        user_stripe_card_id : user_stripe_card_id ? user_stripe_card_id : null,
                        user_wallet_auto_charge_id : (user_wallet_auto_charge_id == "" || !user_wallet_auto_charge_id) ? null : user_wallet_auto_charge_id,
                        net_balance: __USER_BALANCE+(event.data.object.amount/100),
                        status: 'active'
                    })

                    await User.update({user_balance: __USER_BALANCE+(event.data.object.amount/100)}, {where: {id: user_id}})
                }
            ...
            case 'payment_intent.payment_failed': {
                ...
                const __UPDATE_TRANSACTION = await Transaction.update({stripe_payment_intent_transaction_id: event.data.id}, {where: {id: transaction_id}});
                const __FAILED_TRANSACTION_HISTORY = await TransactionHistory.create(
                    {transaction_id, transaction_status: 'failed', status: 'active'}
                );
                break;
            }
            ...
            case 'payout.paid': {
                const {NODE_ENV,user_id, store_id, store_wallet_transaction_id, user_stripe_bank_id, transaction_id} = event.data.object.metadata;

                if(NODE_ENV != process.env.NODE_ENV) {
                    return;
                }

                const __UPDATE_TRANSACTION = await Transaction.update({stripe_payout_transaction_id: event.data.id}, {where: {id: transaction_id}});
                const __SUCCESS_TRANSACTION_HISTORY = await TransactionHistory.create(
                    {transaction_id, transaction_status: 'success', status: 'active'}
                );
                break;
            ...
            case 'payout.canceled': {
                ...
                await returnStripePayout({
                    user_id, store_id, store_wallet_transaction_id, user_stripe_bank_id, transaction_id
                })
...
module.exports = {
    createStripeUser, createUserStripeCard, userWalletRechargeWithCard, 
    stripeWebHook, listUserCards, deleteUserStripeCard, createStoreStripeAccount,
    createStoreStripeBankAccount, createStripePayout, getStoreConnectedAccount,
    createUserStripeBankAccount, listUserBankAccounts, deleteUserBankAccount,
    storeWalletRechargeWithCard,
     
    getUserStripeConnectedAccount, returnStripePayout, stripeDocumentUpload,
